...
## 2026-08-25 合格首稿尾部截断改为局部接写（`bc00c03` 已推送部署；未调用Provider）

- 用户确认重试后，生产 run `d74db417-83db-4be9-b57c-6c2dde4b006e` 的 `write_chapter_draft` 在第2次工作流尝试失败：首稿2524字已处于2200–3000合同内，但 `finish_reason=length` 且以非终止标点结束；2.52错误地把它交给整章压缩，第二稿2749字再次截断。生产只读核对为 `attempt=2`、`generation_version=2.52.0`，没有自动启动其他节点。
- 根因不是字数上限公式，而是修复粒度：合格首稿已有约九成可用正文，整章压缩会重新采样全部剧情并再次消耗约2000 token，既增加长度波动，也可能降低连贯性。
- 2.53.0 对“首稿字数合格但尾部截断”建立独立 `tail_repair` 路径：在结尾前220–820字范围内寻找句末、校验引号/括号闭合，锁定此前正文，只让第二次也是最后一次Provider调用返回320–560字左右的新尾段；合并后仍必须满足2200–3000字和终止完整性。超长首稿继续走整章压缩，过短首稿继续走原有完整重写，不放宽任何门禁。
- 恢复入口只新增一个精确签名：节点必须为2.52.0、`generation_contract`、首稿在范围内、`compress_recover + provider_truncated=True + terminal_reason=non_terminal_punctuation`。2.53新失败、真实超长、质量门或其他终止原因仍不能强制重试。
- 本地回归：生产同型2524字样本锁定2100字前文、只生成500字尾段并合并为2600字；逗号截断反例继续fail-closed。整章/生成/Gateway相关 `154 passed`，single-pass专项 `10 passed`，恢复签名2项通过；`py_compile`和`git diff --check`通过。全量workflow仍在本机注册接口503处受PostgreSQL环境阻断。
- 提交与部署：代码/首轮文档提交 `bc00c03` 已推送 `origin/agent/publishing-v0.9.2`。生产先核对两个运行文件与父提交哈希一致，再精确覆盖并只重建API/Worker/Beat；无迁移、无数据库写入、无前端重建。公网healthz的database/redis/worker正常，API运行时为2.53.0。
- 生产回归：把本提交测试临时复制进容器后single-pass 10项、恢复签名2项通过并删除临时文件。生产目录的旧测试源码仍保留2.52断言，直接运行会出现2项版本断言失败，不能误记为运行时缺陷。旧run水合得到 `retry_after_fix.available=true/from=2.52.0/to=2.53.0`，但仍为 `failed/attempt=2`；该run最后AI调用早于本次部署，证明没有自动重试。
- 清理与回滚：只保留最新源码回滚包 `/opt/deploy-snapshots/starlume-pre-bc00c03-20260825-130412.tar.gz`（140529 bytes）和最新数据库备份 `backups/novelcraft-20260825-100609.sql.gz`；删除旧源码包并清理493.8MB构建缓存后磁盘28G/89G、可用58G（32%）。
- 当前边界：没有再次调用真实Provider，没有新建小说/章节，也没有证明该生产章节已成功、质量门已通过或三章/20章完成；需要用户点击“使用修复版重试章节”后才会进行一次真实生成。
...
本批针对生产 run `108e308e-d058-4e64-ba77-ca863800aabf` 暴露的“首章开场误判、明确剧情承诺被后移、3994字超限、确定性失败反复重试”做了一次调用链收敛，不再通过连续 Provider 试跑猜参数。

- 字数合同：完整正文统一为 `2200-3000` 字；移除内部 `1944-2968` 漂移。Writer 的优先落点为 `2350-2550` 字，首轮输出上限由约3413 token收紧到最多2550 token；超长或有价值的截断候选只允许一次基于原候选的定向压缩/收束，最终仍超限会返回包含首稿/终稿长度和 retry mode 的确定性契约错误。
- 重试边界：章节合同、质量门、输出结构和配置类失败写入 `retryable=false`，Celery 不再整条流程重跑；进度页和后端 retry/restart 接口同时禁止原样重试，只有超时、429、502/503/504、连接错误等暂态 Provider 故障可以重试，且页面一次只重试一个节点。
- 开场判断：动作开场改为检查第一段前160字的真实动作，补齐“拧开/灌/搭上”等常用动作。生产失败样本“实验室空调……林辰拧开保温杯……”现通过动作开场回归，不再因为首句是现场锚点额外调用开场修复。
- 章节承诺：`blueprint_chapter_outline 1.4.0` 新增 `explicit_user_contract/must_deliver/delivery_state/visible_result`；确定性门禁会阻止把“第一章装甲车撞开午门、控制紫禁城、架起慈禧”改成第一章只做实验和计划、第二章才兑现。只在明确出现“第N章”承诺时启用，失败最多做一次聚焦的策划重试，正文尚未生成。
- 界面解释：不再把 `generation_preflight_failed` 与具体根因重复展示；审阅分明确标注为参考分，不能覆盖字数/开场/连续性硬门；确定性失败显示“已停止自动重试”。
- 本地证据：相关后端 `179 passed`；前端全量 `60 passed`；前端生产构建通过；`compileall` 与 `git diff --check` 通过。全量后端基线运行到 `528 passed` 后因本机 Redis 未启动导致认证接口持续503，并伴随既有环境失败，4分07秒时停止，不能标记全量通过。
- 提交与部署：代码及首轮文档提交为 `2fd64ec`，已推送到 `origin/agent/publishing-v0.9.2`。生产目录保留原有脏工作树，未做 reset/pull；按提交文件清单精确覆盖并重建 API/Worker/Beat/Frontend。回滚包为 `/opt/deploy-snapshots/starlume-pre-2fd64ec-20260825-1052.tar.gz`。公网 healthz 返回 `code=0`，database/redis/worker 均正常；生产 API 与前端健康，运行时版本确认 `2.51.0`、字数合同确认 `2200-3000`。生产磁盘 89G/已用27G/可用59G（32%），可用内存约2.1GiB。
- 生产回归：容器内业务目标回归 `172 passed`；另1项源码路径断言因运行镜像没有宿主 `/backend/app/main.py` 而失败，不是业务断言失败。`test_v092_generation_runner.py` 同理由镜像不包含宿主 `/scripts` 而无法收集；两项在本地目标集均已通过。
- 单章真实 Provider：从作品 `11bce2bc-0a6d-4f1d-ab03-6a67fbf74b89` 创建历史章节/V7状态均为0的临时副本，只生成第1章一次。真实 `deepseek-chat` run=`9c35feb4-2959-4cc7-8ce2-ecb55014b348`，chapter=`8c525c74-d0bc-4776-a1b1-c0f37d6e0121`，最终2903字、`status=completed`、V7评分90、generation version `2.51.0`，生成验收 `1/1`。首稿约3644字符，系统只执行一次基于原候选的定向收束，未发生工作流/Celery原样重跑；开场 `action` 契约通过。
- 数据清理：临时副本 `d196efda-a884-4b42-afe8-74c8a2d2414b`、章节、27条trace、7条Prompt执行和全部V7状态已硬删除，数据库确认内容剩余0；源作品未修改。仅保留 `/opt/acceptance-results/v251-single-chapter-20260825.json` 作为验收记录。
- 当前边界：本批证明一次用户触发能在最多一次内部收束后成功，不证明 DeepSeek 首稿必然直接服从字符上限；不证明朱雀95/5/0、生产登录态浏览器、三章/20章、AI披露人工确认或正式发布闭环完成。全量后端本机基线仍受 Redis 未启动影响，不能标记全量通过。

## 2026-08-24 前序全文生成修复记录（历史批次；三章未通过）

## 2026-08-24 整章压缩契约修复与三章真实 Provider 验收（生产已部署；三章生成验收通过）

本批已收敛并修复最后一个章节预算分支：Provider 返回“超长且截断”候选时，旧逻辑会扩大下一次 `max_tokens` 并再次整章生成，导致越写越长；现改为复用候选进入独立终稿压缩调用，压缩调用只接收上一版正文和压缩合同，不再混入原始创作提示。版本为 `CHAPTER_SINGLE_PASS_GENERATION_VERSION=2.50.0`，最多仍为两次 Provider 调用。

- 生产部署：直接部署当前分支代码后，相关确定性回归 `119 passed`；公网 healthz 返回 `code=0`，database/redis/worker 均正常，真实 Provider 配置为 `deepseek`。
- 首次修复后复验副本 `dd929294-ef3d-4054-a277-fe4465e950dd`：第1章生成2811字后通过；第2章首稿超长且截断，暴露了上述漏判分支，停止于第2章，未调用第3章。该副本已软删除，不续跑。
- 最终全新空副本 `6cb195ed-c0e9-41fe-9cc5-c4a15cf1c066` 创建时 `active_chapters=0`、`historical_chapters_cleared=true`，源作品 `4ee9db30-98c7-40d5-9484-12432efed69e` 未清空、未覆盖；generation scope 三章顺序完成：第1章 2938字/V7 89，第2章 2683字/V7 87，第3章 2905字/V7 87，均 `status=completed`，脚本报告 `生成验收通过=3/3`。
- 本次只验收真实生成、正文落库、连续生成和 V7 观察结果；没有宣称七道发布门禁、AI披露人工确认、正式发布回执、朱雀95/5/0或20章完成。20章状态仍为**未开始**；当前三章证据不等于20章质量验收。
- 本批代码与文档已提交为 `e1313b0` 并推送到 `origin/agent/publishing-v0.9.2`；生产运行时代码已部署并经上述测试与三章证据验证。

## 2026-08-24 前序预算修复记录（历史）

本轮针对真实 DeepSeek 长跑逐次定位并完成了三项生成期修复，均已提交到本地分支 `agent/publishing-v0.9.2` 并直接部署生产：

- `b17931e`：预算重写使用已实测的 DeepSeek `0.86` 完成余量；生产回归 `110 passed`。
- `4d93819`：类比密度增加软警告区间；轻度类比记录 warning，不再阻断，明显密集才触发生成重写；生产回归 `111 passed`。
- `b0c43cb`：小于等于800字的预算重写场景提高 token 完成余量；生产回归仍为 `111 passed`。

三次均使用全新副本，创建时均确认 `active_chapters=0` 且 `historical_chapters_cleared=true`，源作品 `4ee9db30-98c7-40d5-9484-12432efed69e` 未清空、未覆盖：

- `b17931e` 运行 `813095b5-3789-4205-808c-a0bdc4227d06`：第1章 2851字、评分89、连续性通过；第2章第3场仍因章节预算越界失败，`accepted=1703,candidate=1345,total=3048`，trace `58edc5853cac4f508b253d83d1b23df5`。
- `4d93819` 运行 `adb8d0ba-34ca-4d06-91af-a13a0f0fc5d8`：第1章第3场类比9处、基线6处；该硬阻断已改为软警告，但本副本失败结果不续跑，trace `e797861081254e5992ea894456171822`。
- `b0c43cb` 运行 `19d3219a-b99a-46c8-95d2-b64be960d359`：第1章第3场在 `token_limit=528,max_chars=614,candidate=717,attempt=3` 截断，trace `bffe195727a448a6b61eb7554295f7ee`。
- `b0c43cb` 运行 `ddbd4d50-b147-4358-af62-11d055e40f6f`：第1章第3场暴露系统性预算分配问题，前两场已接受2152字，第三场候选1338字，合计3490字，并命中 `scene_procedural_motion`，trace `2a61dd40653641599c1a2d4a1c1db465`。

当前结论：生产代码可用、真实 Provider 确实被调用、定向生产回归通过，但三章长跑仍**未通过**。根因已从单个规则收敛为“场景串行预算只保留后续最低字数 + Provider 不严格服从字符上限”：前场可能吃掉章节预算，最后场景无法同时完整收束和满足2200–3000字硬上限。继续调单个检测阈值会重复消耗 Provider，下一步应重构章节级预算分配/整章生成路径后再跑一次受控三章。

本轮仅代码文件直接部署，无数据库结构变更；生产磁盘约99%使用率，未继续创建备份。GitHub 推送在工作站到 GitHub 的 HTTPS 连接上20秒超时，当前不能宣称本轮提交已推送；生产运行时代码哈希已与本地提交文件核对一致。20章仍为**未开始**，AI披露人工确认、正式发布闭环、浏览器登录态验收和朱雀95/5/0仍未验收。

## 2026-08-24 全文候选生产部署与三章受控长跑（已部署；三章未通过）

本批已完成提交、推送、生产备份、迁移和部署，生产最终提交为 `7c78ce5`。部署前备份依次为 `backups/pre-deploy-7c43103-20260824-022558.sql.gz`（305599875 bytes）、`backups/pre-deploy-ba7b5bd-20260824-023934.sql.gz`（305927651 bytes）和 `backups/pre-deploy-7c78ce5-20260824-024621.sql.gz`（305936398 bytes），均通过 `gzip -t`；迁移为 `nc_starlume_authoring (head)`，API/Frontend/PostgreSQL/Redis/Worker/Beat 健康，生产 healthz 返回 `database=ok`、`redis=ok`、`worker=ok`、DeepSeek key 已配置。

- 全文候选生产单章：真实 `deepseek-chat` 返回 HTTP 200，`provider_verified=true`，候选 2921 字，版本 `0a083082-a0ea-483f-814b-55f1e193ccd5`，原第1章正文哈希前后相同；候选独立写入 `versions(entity_type=chapter_draft)`，未覆盖正文。
- 第一次三章实验：全新副本 `f596d4b2-7b6b-4e33-88cf-170b9b935dce` 启动前 `active_chapters=0`；第1章完成（2642字、自动评审91、连续性通过），第2章因开场分类器漏识别“熄/灭”而把句尾“踩”误判为动作开场，触发错误重复门禁并返回500。该问题已由 `ba7b5bd` 修复，失败副本不续跑。
- 第二次三章实验：全新副本 `734fe04e-93da-44b0-9e7f-be9836f898b1` 启动前 `active_chapters=0`；第1章在第3场连续截断（`token_limit=911,max_chars=675,candidate=1211`），未落正文。`7c78ce5` 增加“连续两次明确截断后仅允许一次最终余量重试”，普通质量问题仍保持两次上限；失败副本不续跑。
- 第三次三章实验：全新副本 `fb13d536-a582-42ac-b03d-c3e219e9f1c9` 启动前 `active_chapters=0`；第1章第2场触发 `scene_first_person_narrative`，但日志示例中的“吾”实际来自“支吾了两声”，是单字匹配误报，不是真实第一人称叙述。当前已在本地修正为“吾”仅按词首边界匹配，并补充“支吾”与独立“吾将”回归测试；该失败副本不续跑，修复后需重新部署并新建副本复验。
- 生产回归：开场/截断专项在容器内 `7 passed`；最终三章脚本按 fail-closed 返回退出码2，三次均没有继续到三章，也没有改动源作品 `4ee9db30-98c7-40d5-9484-12432efed69e`。

当前结论：全文候选后端生产链为**可用**，编辑器主入口代码已部署；前三次三章真实 Provider 长跑均未完成，但第三次阻断已定位为确定性检查器误报，当前修复尚未部署，不能把修复后长跑写成通过。20章为**未开始**。AI披露人工确认、七道发布门禁、正式发布闭环、登录态浏览器正式验收和朱雀95/5/0仍不能宣称完成。下一步是部署本次误报修复后，仅新建一个全新空副本复验三章。

## 2026-08-24 编辑器改回全文生成候选（7c78ce5 已部署）
...
- 初次接线时前端全量 `59 passed`、生产构建和差异检查已通过；随后已按上方证据提交、推送并部署。后端真实运行时和单章 Provider 已验证；生产登录态浏览器回归、人工应用候选和正文最终保存仍未完成，不能把三章长跑或朱雀检测写成通过。
