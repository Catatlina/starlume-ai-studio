...
## 2026-08-25 KI-074 字数合格的截断首稿被整章压缩后再次截断（已修复部署）

- 状态：**2.53.0生产可用；`bc00c03` 已推送部署；未再次调用Provider**。
- 生产证据：run `d74db417-83db-4be9-b57c-6c2dde4b006e`、节点 `write_chapter_draft`、`attempt=2`、`generation_version=2.52.0`；首稿2524字、整章压缩稿2749字，二者均在2200–3000内，但都用满token且终稿以非终止标点结束。
- 根因：2.52只区分“截断/未截断”，没有区分“字数已合格、仅尾部残缺”和“整章超长”。前者仍进入 `compress_recover`，导致约2524字有效正文被全部重新生成，第二次2000 token又产生接近完整章节长度的残稿。
- 修复：新增 `tail_repair`。从首稿末尾的小窗口向前寻找语法闭合句界，锁定完整前文；第二次调用只生成结尾替换段，使用独立180–800 token上限。最终统一验证整章字数、句末、引号和括号；失败不再第三次调用。恢复API仅识别该2.52精确错误签名。
- 确定性证据：2524字复现样本锁定2100字并用500字新尾段合并为2600字；同型逗号残尾仍失败。相关生成集154项、single-pass 10项、恢复签名2项通过。
- 生产证据：API/Worker/Beat已重建，公网healthz正常，API运行时2.53.0；当前测试临时进入容器后single-pass 10项、恢复2项通过。旧run已水合出2.52→2.53恢复资格，但保持failed/attempt=2且没有新增AI调用。只保留最新源码回滚包与数据库备份，磁盘可用58G。
- 未闭合：真实Provider结果、V7质量、三章/20章、朱雀和发布闭环均未验收；本机PostgreSQL未启动导致workflow全文件首个注册用例503。生产目录测试源码没有随精确运行文件部署而更新，直接运行旧workflow测试会出现2项旧版本断言失败；当前提交测试已另行在容器通过。
