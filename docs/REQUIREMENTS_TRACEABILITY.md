...
## 2026-08-25 压缩终稿截断语义修复（2.52.0，本地待部署）

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 2200–3000字合同与Provider token预算一致 | 已接线 | DeepSeek默认按1.50汉字/token换算，首轮1834 token；压缩轮使用首稿实测比例；生产同型3540→2803离线回放通过 | 生产运行时版本与真实样本 |
| 字数合格的`length`终稿不被机械拒绝 | 已接线 | 仅第二次`compress_recover`且终端语法完整时继续后续质量门；逗号残句反例仍失败 | 原真实终稿已回滚，无法证明其尾部完整 |
| 失败可诊断且不盲目重跑 | 已接线 | 输出记录终端完整性、首尾token预算、汉字/token比例；本批未调用Provider | 生产错误展示和运行记录复核 |
| 本次后端回归 | 已接线 | 相关链147项通过；全量在注册503处停止 | 启动本机PostgreSQL/Redis后补全量 |
