...
## 2026-08-24 全文生成候选入口（本地已接线，尚未部署）

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 编辑器按作者意图生成2200–3000字完整正文 | 已接线 | `authoring.chapter_draft 1.0.0`、真实 Provider 网关、服务端2200–3000可见字门禁；前端目标请求2600字 | 定向测试、前端构建、生产真实 Provider 单章 |
| AI正文候选不覆盖原稿 | 已接线 | 独立 `versions(entity_type=chapter_draft)`；“填入编辑器”只更新编辑区，主保存前不落库 | 新代码登录态浏览器回归 |
| 人工修改和候选版本保存 | 已接线 | `/drafts/save` 以 `draft_human_edit` 另存版本，仍不改 `contents.body` | 新代码生产回归 |
