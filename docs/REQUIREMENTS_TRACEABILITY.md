...
## 2026-08-25 合格截断首稿局部接写（`bc00c03` 已部署）

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 首稿已在2200–3000时不再整章重写 | 可用 | `tail_repair`锁定语法完整前文，只替换末尾220–820字窗口；2524字回放保留2100字 | 生产真实Provider复验 |
| 章节最多两次Provider调用 | 可用 | Writer一次 + 尾段修复一次；第二次失败直接返回确定性错误 | 生产调用账本核对 |
| 尾段修复不放宽正文合同 | 可用 | 合并稿仍检查2200–3000、终止标点、成对引号/括号；残尾反例失败 | 真实正文后续V7质量门 |
| 2.52旧失败可被修复版恢复 | 可用 | 只匹配2.52、首稿在范围内、非终止标点、`compress_recover`截断签名；生产旧run已水合为2.52→2.53可恢复，2.53和其他失败反例不匹配 | 用户确认后的真实Provider结果 |
| 不静默消耗Provider | 已验收 | 本批生产检查仅只读；旧run仍failed，未创建章节、未调用Provider | 无 |

部署证据：`bc00c03` 已推送并精确部署API/Worker/Beat；运行时2.53.0，公网healthz正常，容器内当前single-pass 10项与恢复2项通过。未部署前端和迁移，旧run保持failed/attempt=2。

## 2026-08-25 修复后安全恢复旧章节失败（`3fed1a4` 已部署）

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 新代码修复后可恢复旧的确定性误判 | 可用 | 只识别字数已合格的pre-2.52 `compress_recover`误拒绝；生产旧run水合出2.52恢复元数据 | 用户点击后的真实Provider结果 |
| 不放开质量/超长/新版确定性失败 | 可用 | 超长反例、`quality_contract`和带`terminal_complete`的2.52失败均不匹配；后端签名回归通过 | 后续生产失败样本复核 |
| 不重跑已成功策划步骤 | 可用 | 同run单节点置pending，页面显示复用12个已成功步骤并隐藏全流程重执行；前端11项通过 | 登录态用户点击确认后的运行记录 |
| 不静默消耗Provider | 已验收 | 部署前后旧run均failed、节点attempt=1；本批没有调用Provider或创建章节 | 无 |
| 可审计与可恢复分发 | 可用 | `context.code_fix_retries`保留旧错误/版本/时间；复用`dispatch_bootstrap_run`，Broker失败落`dispatch_failed` | 生产真实分发样本 |
| 全量后端基线 | 未验收 | 定向后端2项、Python编译和强制门禁通过 | 本机PostgreSQL启动后补全量 |

## 2026-08-25 压缩终稿截断语义修复（`114bee6` 已部署）

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 2200–3000字合同与Provider token预算一致 | 可用 | 生产API/Worker 2.52.0；DeepSeek默认按1.50汉字/token换算，首轮1834 token；压缩轮使用首稿实测比例；生产同型3540→2803离线回放通过 | 后续自然发生的真实样本 |
| 字数合格的`length`终稿不被机械拒绝 | 可用 | 仅第二次`compress_recover`且终端语法完整时继续后续质量门；容器回归8项通过，逗号残句反例仍失败 | 原真实终稿已回滚，无法证明其尾部完整 |
| 失败可诊断且不盲目重跑 | 可用 | 输出记录终端完整性、首尾token预算、汉字/token比例；本批未调用Provider | 后续生产运行记录复核 |
| 本次后端回归 | 可用 | 本地相关链147项、生产容器整章8项、强制真实性门禁通过；全量在注册503处停止 | 启动本机PostgreSQL/Redis后补全量 |

## 2026-08-25 首章生成合同、重试和质量修复（`2fd64ec` 已部署）

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 完整正文严格落在2200–3000字 | 可用 | 运行时 `2.51.0`；硬合同不再缩成1944–2968；临时空副本真实第1章2903字，generation验收1/1 | 多题材样本；DeepSeek首稿仍可能需要一次内部收束 |
| 确定性失败不再反复消耗Provider | 可用 | 质量/长度/结构/配置失败写 `retryable=false`；Celery、retry/restart API、前端共同阻止原样重跑；旧run曾累计20次，新实现仅允许暂态故障重试 | 生产登录态按钮行为验收 |
| 动作开场不误判现场锚点 | 可用 | 前160字动作识别回归；真实单章 `requested=action/observed=action/passed=true` | 更多开场类型样本 |
| 明确第N章剧情承诺当章兑现 | 可用 | `blueprint_chapter_outline 1.4.0` 与确定性承诺覆盖门；目标回归覆盖“首章拿下紫禁城不能后移” | 全新bootstrap真实Provider样本尚未执行 |
| 一次用户触发完成一章 | 可用 | 真实 `deepseek-chat` run `9c35feb4...`，2903字、V7 90、completed；首稿超长后只做一次候选收束，无工作流原样重跑 | 不能宣称单次Provider调用必过 |
| 测试数据不污染书库 | 已验收 | 临时副本 `d196efda...` 及章节、V7状态、Prompt轨迹已硬删除，DB剩余0；源作品未修改 | 无 |
| 朱雀95/5/0与发布闭环 | 未验收 | 本批未调用朱雀，generation scope 不运行发布门禁 | 外部检测、披露人工确认、七道门禁、正式发布回执 |
...
## 2026-08-25 首章一次成功与质量合同追踪

| 需求 | 状态 | 当前证据 | 未闭合门禁 |
|---|---|---|---|
| 完整正文统一2200–3000字 | 可用 | `reader_chapter_budget` 不再产生1944–2968；首轮优先2350–2550、最多2550 token；超限只做一次候选压缩 | 真实Provider单章字数与收束完整性 |
| 确定性失败不反复消耗Provider | 可用 | 任务持久化 `retryable=false`；retry/restart API 409；前端无重试按钮且一次只重试一个可恢复节点 | 生产任务队列与登录态浏览器验收 |
| 动作开场不因短现场锚点误判 | 可用 | 生产实验室样本回归通过；检查第一段前160字动作 | 不同题材真实开场样本 |
| 用户明确第N章事件不得后移 | 可用 | 细纲Prompt 1.4.0 + 确定性章节承诺覆盖检查；坏/好样本回归 | 新书真实策划输出验收 |
| 失败页显示根因而非级联噪声 | 可用 | 过滤派生 `generation_preflight_failed`；审阅分标注为参考分；显示停止自动重试 | 生产浏览器截图验收 |
| 本批全量与生产验收 | 已接线 | 定向后端179项、前端全量60项和构建通过 | 本机Redis阻断全量；未commit/push/deploy/Provider复验 |
