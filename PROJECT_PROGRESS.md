...
## 2026-08-25 2.53.0 合格截断首稿尾段修复（`bc00c03` 已部署）

- 生产失败已只读确认：run `d74db417-83db-4be9-b57c-6c2dde4b006e` 的章节节点为 `attempt=2`、2.52.0；2524字合格首稿尾部截断后被整章压缩为2749字，第二稿仍截断。没有自动继续其他节点。
- 已实现章节局部尾段接写：锁定语法闭合前文，只生成一个有独立小token预算的替换尾段；总调用仍最多两次。合并稿继续执行2200–3000字、终止完整性和后续V7门禁。
- 已增加2.52精确恢复签名，允许部署后在同run只重试章节节点；其他确定性失败仍拒绝。
- 当前证据：生成相关154项、single-pass 10项、恢复签名2项通过，编译和差异检查通过。workflow全文件因本机注册503环境阻断，不能标记全量通过。
- 提交与生产：`bc00c03` 已推送并精确部署API/Worker/Beat；运行时2.53.0，公网healthz正常。容器使用当前测试文件回归10+2项通过，旧run显示2.52→2.53恢复资格但保持failed/attempt=2，部署未调用Provider。
- 回滚与资源：只保留最新源码回滚包和数据库备份；构建缓存清理493.8MB后磁盘可用58G。没有迁移、前端变更或数据库业务写入。
- 当前状态：修复链为生产可用；没有真实Provider复验，不宣称章节成功或质量验收。
